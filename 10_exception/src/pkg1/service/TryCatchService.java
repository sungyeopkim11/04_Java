...
public class TryCatchService {
	...
	/* Exception(예외) : 코드를 해결 가능한 에러
	 *   ( if, try-catch, trows 등)
	 *   
	 *   
	 *   
	 * Exception Handilng(예외 처리)
	 * 예외 처리
	 * 
	 */
...
//		str.split("/");
	...
	public void test4() {
		...
		// -> 부모 타입을 나머지 예외를 처리하는(else) 형식으로 사용
			...
			System.out.println("int 범위 정수만 입력 해줘");
		
		
		} 
		catch(RuntimeException e) {
			// catch에 매개 변수 타입을 부모 타입으로 설정
			// -> 던져지는 모든 자식 타입의 예외를 잡아소 처리
			// -> 예외 처리에 다형성(업 캐스팅) 적용
